...
class PartVQEncoder(nn.Module):
    ...
    def from_latent_to_fea(self, x, path=False):
        zs = self.bottleneck.encode(x)
        zs = self.bottleneck.decode(zs)
        if path:
            zs[0] = x[0] + (zs[0] - x[0]).detach()
        out = self.postprocess(zs[0]).permute(1, 0, 2).contiguous()
    ...
    def forward(self, x):
        ...
        return zs, xs_quantised, commit_losses, quantiser_metrics
