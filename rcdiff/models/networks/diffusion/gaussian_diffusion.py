...
import development.models.networks.diffusion.rotation_conversions as geometry


def _loss_fn(x_pred, x_tgt, robust=True):
    if robust:
        loss = F.smooth_l1_loss(x_pred, x_tgt, reduction='none')  # Huber
    else:
        loss = (x_pred - x_tgt).abs()
    return loss.mean()
...
def get_joints24(joint3d):
    device = joint3d.device
    joint3d24 = torch.zeros(joint3d.shape[0], joint3d.shape[1], 24, 3, device=device)
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.regennet_loss = regennet_loss
        ...
        # self.loss_bce = th.nn.BCEWithLogitsLoss()
    ...
    def p_mean_variance(
        ...
        B, C = x.shape[:2]
        assert t.shape == (B,)
        model_output, pred_pose, pred_rot, pred_root_vel, pred_transl, pred_contact \
        ...
        if self.model_mean_type == ModelMeanType.PREVIOUS_X:
            ...
            pred_xstart = process_xstart(self._predict_xstart_from_v(x, t, model_output))
            model_mean, _, _ = self.q_posterior_mean_variance(
                x_start=pred_xstart, x_t=x, t=t
            )
        else:
    ...
    def _predict_xstart_from_v(self, x_t, t, v):
        # x0 = sqrt(alphabar)*x_t - sqrt(1-alphabar)*v
        s_ab = _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape)
        s_omab = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
        return s_ab * x_t - s_omab * v

    def _predict_v(self, x_start, t, noise):
        # v = sqrt(alphabar)*eps - sqrt(1-alphabar)*x0
        s_ab = _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_start.shape)
        s_omab = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_start.shape)
        return s_ab * noise - s_omab * x_start
    ...
    def ddim_sample_loop_progressive(
        ...
        if noise is not None:
            ...
            img = th.randn(*shape, device=device)
        num_inference_steps = self.num_timesteps - skip_timesteps
        indices = np.linspace(0, self.num_timesteps - 1, num_inference_steps, dtype=int)[::-1]
    ...
    def training_losses(self, model, x_start, t, cmx=None, model_kwargs=None, noise=None, scale_transl=False):
        """
        ...
        :param cmx: condition sequence for diffusion training.
        :param model_kwargs: extra keyword arguments for optional sampling-style calls.
        ...
        if self.loss_type == LossType.KL or self.loss_type == LossType.RESCALED_KL:
            ...
            if cmx is not None:
                model_output, _, _, _, _, _ = model(x_t, self._scale_timesteps(t), cmx=cmx)
            else:
                model_output, _, _, _, _, _ = model(x_t, self._scale_timesteps(t), **model_kwargs)
            ...
            terms["diff_mse"] = self.loss_mse(target, model_output)  # mean_flat(rot_mse)

            terms["loss"] = terms["diff_mse"] + terms.get("vb", 0.)
        else:
